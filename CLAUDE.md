...
codeforge ζ arc Requirements lane plugin. 7 agent 병렬 (PL + 6 sub: Domain·Analyst·Researcher + ChangeImpact·Feasibility·Continuity) + 도메인 KB owner write.
...
| `docs/stories/<KEY>.md §5` (요구사항 확장 해석 — ambiguity-driven) | RequirementsPLAgent (Analyst 결과 통합) | `Edit(docs/stories/**)` |
| `docs/stories/<KEY>.md §6` (개념 정립 + 외부 지식 + concept-driven 재편) | RequirementsPLAgent (Researcher 결과 통합) | `Edit(docs/stories/**)` |
| `docs/stories/<KEY>.md §4.1` (코드 변경 델타) | ChangeImpactAgent (write queue drain) | `.claude-work/doc-queue/` |
| `docs/stories/<KEY>.md §4.2` (구현 가능성) | FeasibilityAgent (write queue drain) | `.claude-work/doc-queue/` |
...
| Sub-agent | Reshape dimension | Mandate | Story §N owner |
|---|---|---|---|
| DomainAgent | known knowns (사내 지식 기반 해석) | 사내 도메인 + ADR + 코드베이스 기반 해석 | §2 |
| RequirementsAnalyst | ambiguity-driven (원문 모호성 확장) | edge case + implicit constraint + assumption 확장 | §5 |
| ResearcherAgent | concept-driven (외부 unknown unknowns + 개념 명시화) | Concept formulation + Deep exploration + Requirement reshape (concept-driven) | §6 |
| ChangeImpactAgent | code-delta (src/** 변경 범위) | src/** 전체 → AS-IS/DELTA 매핑 | §4.1 |
| FeasibilityAgent | feasibility (아키텍처 가능성) | src/** + ADR → 구현 가능성 등급 + 경고 힌트 | §4.2 |
| ContinuityAgent | continuity (이전 작업 연속성) | docs/stories·change-plans·ADR → 충돌/중복/의존 분류 | §4.3 |
